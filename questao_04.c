...
double volume = 0.5; // valor de 0.1 à 1.0
/* USER CODE END PV */
...
int main(void) {
	...
	while (1) {
		for (uint32_t i = 0; i < AUDIO_DATA_LEN_1; i++) {
			DAC_SetValue(DAC_CHANNEL2, (uint8_t) specialz[i]*volume, DAC_RES_8BITS);
			Delay_us(21);
		}
...
void TIM5_IRQHandler() {
	TIM5->SR &= ~TIM_SR_UIF;
//
//	DAC_SetValue(DAC_CHANNEL2, specialz[audio_index], DAC_RES_8BITS);
//
//	audio_index++;
//
//	if (audio_index >= AUDIO_DATA_LEN_1) {
//		audio_index = 0;
//	}
}
/* USER CODE END 4 */
