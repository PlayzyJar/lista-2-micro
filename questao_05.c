...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
void acionar_servo_1(uint16_t duty) {
...
void acionar_servo_2(uint16_t duty) {
...
uint16_t map_adc_to_duty(uint16_t adc_value) {
	return 500 + ((adc_value * 2000) / 4095);
}
...
int main(void) {
	...
	while (1) {
		...
		jstck_2 = ADC_GetSingleConversion(ADC2);

		acionar_servo_1(map_adc_to_duty(jstck_1));
		acionar_servo_2(map_adc_to_duty(jstck_2));
